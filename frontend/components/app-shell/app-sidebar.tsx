...
import { ChevronsUpDown, GitBranch, History, Mic, PanelLeftClose, PanelLeftOpen, Table2 } from "lucide-react";
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      {badge != null && badge > 0 && (
        <span className={cn("min-w-[18px] rounded bg-zinc-950 px-1.5 py-0.5 text-center text-[10px] font-semibold leading-none text-white tabular-nums", collapsed && "absolute ml-6 mt-[-18px]")}>
          {badge}
        </span>
      )}
    </Link>
...
export function AppSidebar({ projectId, pendingCount = 0 }: Props) {
  ...
  return (
    <aside className={cn("flex shrink-0 flex-col border-r border-zinc-200 bg-white py-5 transition-[width] duration-200", collapsed ? "w-[72px] px-3" : "w-[240px] px-4")}>
      <div className={cn("mb-7 flex items-center gap-2", collapsed ? "justify-center" : "justify-between")}>
        ...
        <button
          ...
          className="grid h-8 w-8 place-items-center rounded-md text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900"
      ...
      <nav className="flex-1 space-y-5 overflow-y-auto">
        <div>
          ...
          <div className="space-y-0.5">
            ...
            <NavItem href={routes.meetingNote(projectId)} icon={Mic} label="회의록 분석" active={pathname === "/meeting-note"} badge={pendingCount} collapsed={collapsed} />
